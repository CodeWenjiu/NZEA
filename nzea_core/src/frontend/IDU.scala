...
/** IDU decode result: pc, pred_next_pc, imm, rd_index, physical regs, fu_type, fu_op, fu_src, csr_addr, csr_will_write.
  * `is_ret` (RAS/sim ret classification) is a config-derived information unit; `rd_index` stays unconditional because
  * the commit chain (ROB/DPI) consumes it regardless of RAS.
  */
class IDUOut(width: Int, prfAddrWidth: Int)(implicit config: CoreConfig) extends Bundle {
  ...
  val is_ret =
    if (PayloadSpec.enabled(PayloadSpec.RetExec)) Some(Bool()) else None // JALR returning via x1 (BRU ret stats)
...
class IDU(addrWidth: Int)(implicit config: CoreConfig) extends Module {
  ...
  when(io.out.flush) {
    ...
    when(
      io.commit.valid && !io.out.flush && io.commit.bits.rd_index =/= 0.U &&
  ...
  io.out.bits.csr_will_write := (fuType === FuType.SYSU) && MuxCase(
    false.B,
    Seq(
