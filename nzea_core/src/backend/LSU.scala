...
/** LSU wrapper: unifies AGU->LSQ write path, LSQ queueing/issue, and MemUnit bus/resp handling. Internal behavior is
  * unchanged; this module only consolidates boundaries.
  */
class LSU(
    width: Int,
    robIdWidth: Int,
  ...
  private val mem = Module(new MemUnit(width, robIdWidth, prfAddrWidth, mmioRanges))
