...
class BRU(robIdWidth: Int, prfAddrWidth: Int) extends Module {
  ...
  val is_jalr = bruOpU(1)  // JALR is the only op that uses rs1 for target
  ...
  val u = Rob.entryStateUpdate(io.in.valid, b.rob_id, is_done = true.B, flush = mispredict, next_pc = next_pc)(robIdWidth)
