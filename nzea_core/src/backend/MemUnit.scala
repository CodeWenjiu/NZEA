...
/** MemUnit: receives mem_req from LSQ, pass-through to dbus (same-cycle, no timing change). Handles resp: rob_access
  * (is_done), WBU (load only). No unaligned support.
  */
class MemUnit(
    width: Int,
    robIdWidth: Int,
  ...
  // Detect MMIO from injected device ranges (tile/platform layer provides these).
  private val addr = req.addr

  private val reqIsMmio: Bool =
    if (mmioRanges.isEmpty) false.B
    else {
      mmioRanges
        .map { case (base, size) =>
  ...
  io.rob_access <> Rob.entryStateUpdate(respFire, respUser.rob_id, is_done = true.B, is_mmio = respUser.is_mmio)(
    robIdWidth
  )
