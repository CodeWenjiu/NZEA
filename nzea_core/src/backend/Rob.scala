...
/** Head output: rob_id, rob_state, rd_index, rd_value, next_pc (BRU writes; else pred_next_pc from enq), flush. */
...
/** Commit info for WBU: only when head is Done. next_pc, rd_index, rd_value, flush. */
...
/** GPR bypass payload: rd and data for ISU to bypass when oldest Done entry commits. */
class GprBypass extends Bundle {
  val rd   = UInt(5.W)
  val data = UInt(32.W)
}

/** ROB entry state update: rob_id, new_state, rd_value. For BRU: flush, next_pc. For AGU WaitingForMem: mem_* */
...
/** FU output to Rob: valid/bits from FU, ready/flush from Rob. Compatible with PipeIO for <> with FU. */
...
/** Standard FU output: PipeIO or RobAccessIO. FUs output PipeIO; Rob connects with ready=1, flush forwarded. */
object Rob {
  type FuOutput = RobAccessIO

  def entryStateUpdate(valid: Bool, rob_id: UInt, new_state: RobState.Type, rd_value: UInt,
                       flush: Bool = false.B, next_pc: UInt = 0.U,
                       mem_addr: UInt = 0.U, mem_wdata: UInt = 0.U, mem_wstrb: UInt = 0.U,
                       mem_lsuOp: LsuOp.Type = LsuOp.LB, mem_pred_next_pc: UInt = 0.U)(idWidth: Int): Valid[RobEntryStateUpdate] = {
...
/** Rob: depth-entry circular buffer. rob_id = stable buffer address.
  * Call connectFuOutputs(fuOutputs) to wire FU outputs; uses Valid[RobEntryStateUpdate] as standard. */
class Rob(depth: Int, numAccessPorts: Int) extends Module {
  ...
  /** Wire FU outputs to access ports. Rob drives ready=1, flush when commit.fire && head.flush. */
