...
/** LS_Queue alloc IO: producer (ISU) drives valid/bits, consumer (MemUnit) drives ready/lsq_id. Use Flipped for
  * producer.
  */
...
/** Unified Rob slot read: all fields any consumer may need. ISU: valid, is_done. Mem data in LS_Queue. rd_value from
  * PRF(p_rd) at commit.
  */
...
/** FU output to Rob: state update for an entry. Mem data (addr,wdata,wstrb) goes to LS_Queue. rd_value not stored in
  * Rob; commit reads from PRF(p_rd). mem_type set at enq (ISU), not updated by FU. csr_type/csr_data: when SYSU writes
  * CSR, set at completion; CsrType.None otherwise.
  */
