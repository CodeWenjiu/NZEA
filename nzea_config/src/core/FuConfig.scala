...
/** FU config: describes whether each FU needs PRF write, bypass, Rob access, etc. Used for config-driven parameterized
  * architecture: port count derived from config, top-level uses foreach for wiring.
  */
case class FuConfig(
    kind: FuKind,
...
object FuConfig {
  ...
  /** PRF write source list derived from ISA config. Order: ALU, BRU, SYSU, [MUL, DIV], [NNU], MemUnit. Ports with
    * hasBypass=true participate in operand bypass.
    */
  ...
  /** Rob access port list derived from ISA config. Order: ALU, BRU, SYSU, [MUL, DIV], [NNU], AGU. Port count depends on
    * hasM / hasWjcus0.
    */
  ...
  /** Issue port list: order ALU, BRU, AGU, [MUL, DIV], [NNU], SYSU. Port index = hardware Vec index. Each port supports
    * one FuType. Used for mask-based routing in ISU.
    */
